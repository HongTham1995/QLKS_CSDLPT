...
namespace DAO
{
    public class Database
    {
        ...
        public Database()
        {
            // Sử dụng luôn centralConnString, vì truy vấn sẽ dùng linked server để kết nối đến các chi nhánh.
            conn = new SqlConnection(centralConnString);
        }
        ...
        public List<TaiKhoanDTO> GetListTK_DTO(string query)
        {
            ...
            while (reader.Read())
            {
                ...
                tk.MaNV = reader[1] != DBNull.Value ? reader[1].ToString() : "";
                tk.MatKhau = reader[2].ToString();
                tk.TinhTrang = int.Parse(reader[3].ToString());
                tk.MaPQ = reader[4] != DBNull.Value ? reader[4].ToString() : "";
