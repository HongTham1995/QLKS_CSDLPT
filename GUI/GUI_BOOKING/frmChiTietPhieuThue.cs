...
namespace GUI.GUI_BOOKING
{
    public partial class frmChiTietPhieuThue : Form
    {
        ...
        {
            tbRoom.Rows.Clear();
            List<ChiTietThuePhongDTO> cttps = cttP.GetDSListCTTP(sever,maCTT);
        ...
        private void HienThiChiTietThueDichVu()
        {
            ...
            List<ChiTietThueDichVuDTO> cttdvs = cttDV.GetListChiTietDichVu(sever,maCTT);
        ...
        private void tbRoom_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == index && e.RowIndex >= 0)
            {
                ...
                if (ans == MessageBoxDialog.YES_OPTION)
                {
                    ...
                    {
                        cttP.DeleteCTTP(sever,maCTT, tbRoom.SelectedRows[0].Cells[1].Value.ToString(), DateTime.Parse(tbRoom.SelectedRows[0].Cells[5].Value.ToString()).ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        private void lấyPhòngToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (tbRoom.SelectedRows.Count > 0)
            {
                if (tbRoom.SelectedRows[0].Cells[3].Value.ToString().Equals("Đang xử lý"))
                {
                    ...
                    cttP.UpdateTinhTrang(sever, maCTT, maP, ngayThue, "1");
                    phong.SuaTinhTrang(sever,maP, "1");
                    MessageBoxDialog messageError = new MessageBoxDialog();
        ...
        private void trảPhòngToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (tbRoom.SelectedRows.Count > 0)
            {
                if (tbRoom.SelectedRows[0].Cells[3].Value.ToString().Equals("Đang được thuê"))
                {
                    string maP = tbRoom.SelectedRows[0].Cells[1].Value.ToString();
                    string ngayThue = DateTime.ParseExact(tbRoom.SelectedRows[0].Cells[5].Value.ToString().Trim(), "dd/MM/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
                    cttP.UpdateTinhTrang(sever, maCTT, maP, ngayThue, "2");
                    if (!tbRoom.SelectedRows[0].Cells[4].Value.ToString().Equals("Khác"))
                    {
                        cttP.UpdateCheckOut(sever, true, maCTT, maP, ngayThue, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "0");
                    }
                    else
                    {
                        var item = from cttPhong in cttP.GetDSListCTTP(sever)
                        ...
                        cttP.UpdateCheckOut(sever, false, maCTT, maP, ngayThue, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), gia.ToString());
                    }
                    phong.SuaTinhTrang(sever, maP, "2");
                    MessageBoxDialog messageError = new MessageBoxDialog();
        ...
        private void tbService_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == index && e.RowIndex >= 0)
            {
                ...
                if (ans == MessageBoxDialog.YES_OPTION)
                {
                    cttDV.DeleteCTTDV(sever,maCTT, tbService.SelectedRows[0].Cells[1].Value.ToString(), DateTime.Parse(tbService.SelectedRows[0].Cells[3].Value.ToString()).ToString("yyyy-MM-dd"));
                    MessageBoxDialog messageError = new MessageBoxDialog();
            ...
            if (e.ColumnIndex == indexUpdate && e.RowIndex >= 0)
            {
                ...
                if(frmInput.DialogResult == DialogResult.Yes)
                {
                    ...
                    cttDV.SuaSoLuongCTTDV(sever,maCTT, tbService.SelectedRows[0].Cells[1].Value.ToString(), DateTime.Parse(tbService.SelectedRows[0].Cells[3].Value.ToString()).ToString("yyyy-MM-dd"), soLuong.ToString());
                    HienThiChiTietThueDichVu();
                    MessageBoxDialog messageError = new MessageBoxDialog();
