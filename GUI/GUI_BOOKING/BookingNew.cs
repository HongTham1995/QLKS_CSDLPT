...
namespace GUI.GUI_BOOKING
{
    public partial class BookingNew : Form
    {
        ...
        {
            InitializeComponent();
            rooms = phong.getListPhong_DTO(sever);
        }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
                ...
                else
                {
                    ...
                    if (radioButton1.Checked)
                    {
                        ...
                        else
                        {
                            ...
                            PhongBUS phong = new PhongBUS();
                            var cttList = from cttp in cttPhong.GetDSListCTTP(sever)
                                          join ctt in ctThue.getDSChiTietThue(sever) on cttp.MaCTT equals ctt.MaCTT
                                          join room in phong.getListPhong_DTO(sever) on cttp.MaP equals room.MaP
                                          where ctt.XuLy == 0 && ctt.TinhTrangXuLy == 0 && cttp.NgayThue <= dtNgayTra.Value && (cttp.NgayTra >= dtNgayThue.Value || !cttp.NgayTra.HasValue)
                                          select room;
                            var roomsValid = phong.getListPhong_DTO(sever).Where(room => room.TinhTrang == 0 && !cttList.ToList().Any(ctt => ctt.MaP == room.MaP));
            ...
            else
            {
                ...
                List<PhongDTO> list = phong.getListPhong_DTO(sever);
