        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (dt.Rows[i][3].ToString().ToUpper().Equals("THEO NGÀY"))
                    cttp.InsertCTTP(sever,true, maCTT.Trim(), dt.Rows[i][0].ToString().Trim(), DateTime.Parse(dt.Rows[i][4].ToString().Trim()).ToString("yyyy-MM-dd HH:mm:ss"), DateTime.Parse(dt.Rows[i][5].ToString().Trim()).ToString("yyyy-MM-dd HH:mm:ss"), "0", dt.Rows[i][7].ToString());
                else if (dt.Rows[i][3].ToString().ToUpper().Equals("THEO GIỜ"))
                    cttp.InsertCTTP(sever,true, maCTT.Trim(), dt.Rows[i][0].ToString().Trim(), DateTime.Parse(dt.Rows[i][4].ToString().Trim()).ToString("yyyy-MM-dd HH:mm:ss"), DateTime.Parse(dt.Rows[i][5].ToString().Trim()).ToString("yyyy-MM-dd HH:mm:ss"), "1", dt.Rows[i][7].ToString());
                else
                    cttp.InsertCTTP(sever,false, maCTT.Trim(), dt.Rows[i][0].ToString().Trim(), DateTime.Parse(dt.Rows[i][4].ToString().Trim()).ToString("yyyy-MM-dd HH:mm:ss"), DateTime.Parse(dt.Rows[i][4].ToString().Trim()).ToString("yyyy-MM-dd HH:mm:ss"), "2", dt.Rows[i][7].ToString());
            }
            MessageBoxDialog message = new MessageBoxDialog();
